...
class CompletionRefresher(object):
    ...
    def refresh(self, executor, special, callbacks):
        """
        Creates a VCompleter object and populates it with the relevant
        completion suggestions in a background thread.

        executor - VExecute object, used to extract the credentials to connect
                   to the database.
        special - VSpecial object used for creating a new completion object.
        ...
        if self.is_refreshing():
            ...
            return [(None, None, None, 'Auto-completion refresh restarted.', True)]
        else:
    ...
    def _bg_refresh(self, vexecute, special, callbacks):
        completer = VCompleter(smart_completion=True, vspecial=special)

        # Create a new vexecute method to popoulate the completions.
        e = vexecute
        executor = VExecute(e.dbname, e.user, e.password, e.host, e.port)
